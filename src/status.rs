//! `vs-connect status`: one command that makes sense on either machine —
...
pub fn run() -> Result<()> {
    println!("vs-connect {}", env!("CARGO_PKG_VERSION"));
    ...
    if sock.exists() {
        match daemon::ping(&sock) {
            ...
            _ => println!(
                "  socket {} exists but nothing answered (stale) — `vs-connect daemon` will clean it up",
                sock.display()
            ),
        }
    } else {
    ...
    match std::env::var("SSH_AUTH_SOCK") {
        ...
        Ok(s) => match daemon::ping(Path::new(&s)) {
            Ok(Some(r)) => println!(
                "  SSH_AUTH_SOCK reaches a vs-connect daemon (pid {}) — `code <path>` will open \
                 windows on that machine",
                r.pid
            ),
            Ok(None) => println!(
                "  SSH_AUTH_SOCK reaches an ssh-agent that is not vs-connect (normal on your \
